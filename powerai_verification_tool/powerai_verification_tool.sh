...
then
    ...
    if version_ge $installed_version $required_version; then
        echo " $installed_version is not equal to $required_version." 2>&1 >> "${LOGFILE}"
    else
...
then
   ...
   echo " Optional repos must be enabled in order to install PowerAI." 2>&1 | tee -a "${LOGFILE}"
   IS_OPTIONAL_REPO_ENABLED="NO"
fi
...
then
    ...
    echo " Extras repo must be enabled in order to install PowerAI." 2>&1 | tee -a  "${LOGFILE}"
    IS_EXTRAS_REPO_ENABLED="NO"
fi
...
then
    ...
    echo " EPEL repo must be enabled in order to install PowerAI." 2>&1 | tee -a "${LOGFILE}"
    IS_EPEL_REPO_ENABLED="NO"
fi
...
then
   ...
   then 
       ...
       then
          echo " CUDA $current_cuda_version is not equal to $cuda_version."  2>&1 >> "${LOGFILE}"
       else
...
validate_anaconda(){
       ...
       if [ "${ANACONDA_VER}" != "0" ]
       then
          echo " Anaconda is installed successfully."  2>&1 | tee -a "${LOGFILE}"
      ...
      anaconda_version1=$(conda list | grep "^conda " | awk '{print $2}')
      if [ "$anaconda_version1" = "$anaconda_ver" ]
      then
          echo " Anaconda version: ${anaconda_ver} configured successfully." 2>&1 | tee -a "${LOGFILE}"
          echo " Anaconda is installed successfully."  2>&1 >> "${REPFILE}"
          echo " Anaconda version: ${anaconda_ver} configured successfully." 2>&1 >> "${REPFILE}"
      else
...
echo "Create and Activate py3 environment" {
if ! conda-env list | grep '^py36' | grep 'py36$' ; then
   conda create -y --name py36 python=3.6 anaconda
fi
...
if [ $? -eq 0 ]
then
   echo " mldl-repo-local installed successfully" 2>&1 >> "${LOGFILE}"
...
if [ $? -eq 0 ]
then
   echo " power-mldl installed successfully" 2>&1 >> "${LOGFILE}"
...
if [ -e $LICENSE_BIN/check-powerai-license.sh ]; then
  ...
  if [ $? -ne 0 ]
  then
      echo " PowerAI license has not been accepted." 2>&1 | tee -a  "${LOGFILE}"
      POWERAI_LICENSE_ACCEPTED="NO"
      cleanup_and_exit
  else
     echo " PowerAI license is accepted." 2>&1 >> "${LOGFILE}"
     echo " PowerAI installed successfully" 2>&1 | tee -a "${REPFILE}"
...
validate_ddl(){
   ...
   # If ddl is installed, on PowerAI we need to activate it
   if [ "${IS_DDL_INSTALLED}" != "0" ]
   then
       ...
       then
           ...
           if [ $? -eq 0 ]
           then
               echo " DDL activated successfully" 2>&1 >> "${LOGFILE}"
               IS_DDL_INSTALLED_REP="YES"
           else
  ...
  if [ "${IS_DDL_INSTALLED}" != "0" ]
   then
        ...
        then
           ...
           # echo " DDL $ddl_version installed successfully"
        fi
    fi
...
#    echo "DDL version: $ddl_version1" 2>&1 > "${REPFILE}"
    ...
    then
       echo " DDL version: ${ddl_version}" installed successfully. 2>&1 | tee -a "${REPFILE}"
    else
...
validate_snapml(){  
   ...
   then
       ...
       then
           ...
           if [ $? -eq 0 ]
           then
              echo " SNAPML activated successfully" 2>&1 >> "${LOGFILE}"
              IS_SNAPML_INSTALLED_REP="YES"
           else
  ...
  if [ "${IS_SNAPML_INSTALLED}" != "0" ]
   then
    ...
    then
       echo " SNAPML version: ${snapml_version}" installed successfully. 2>&1 | tee -a "${REPFILE}"
    else
...
validate_tensorflow(){  
   ...
   then
       ...
       then
           ...
           if [ $? -eq 0 ]
           then
              echo " Tensorflow activated successfully" 2>&1 >> "${LOGFILE}"
              IS_TENSORFLOW_INSTALLED_REP="YES"
           else
              /opt/DL/tensorflow/bin/install_dependencies -y
              source /opt/DL/tensorflow/bin/tensorflow-activate
   ...
   then
    ...
    then
       echo " Tensorflow version: ${tf_version}" installed successfully. 2>&1 | tee -a "${REPFILE}"
    else
...
validate_caffe() {
    ...
    then
        ...
        if [ $? -eq 0 ]
        then
           echo " Caffe activated successfully" 2>&1 >> "${LOGFILE}"
        else
    ...
    then
       echo " IBM Caffe version: ${caffe_version}" installed successfully. 2>&1 | tee -a "${REPFILE}"
    else
...
validate_pytorch(){  
   ...
   # If pytorch is installed, on PowerAI we need to activate it
   if [ "${IS_PYTORCH_INSTALLED}" != "0" ]
   then
       if [ $POWERAI_LICENSE_ACCEPTED = "YES" ]
       then
           ...
           if [ $? -eq 0 ]
           then
                echo " PYTORCH activated successfully" 2>&1 >> "${LOGFILE}"
                IS_PYTORCH_INSTALLED_REP="YES"
           else
  ...
  then
        ...
        then
            ...
            #echo " pytorch $pytorch_version installed successfully"
        fi
    fi
    ...
    then
       echo " Pytorch version: ${pytorch_version}" installed successfully. 2>&1 | tee -a "${REPFILE}"
    else
