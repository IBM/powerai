...
This version of deeplab has been enabled to run with TensorFlow Large Model
Support. This requires a corresponding change to the
tensorflow/contrib/slim/python/slim/learning.py to allow the LMS instance
to run on the model after SLIM has finished building it up in the
slim.learning.train method. The changes to learning.py can be found in
learning.py.patch. These changes can be applied to learning.py with the
following command:
```
patch -p1 learning.py < learning.py.patch
```
